...
import tensorflow as tf2     # 没有会话机制，直接计算
import tensorflow.compat.v1 as tf   # 使用v1否则很多地方会报错
from tensorflow.python.framework import ops
from M2W3 import tf_utils
import time
import os

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'    # 去掉一个AVX警告，看着烦
tf.disable_eager_execution()    # 不加这一行的话就必须要在会话里定义所有的变量
...
#
# init = tf.global_variables_initializer()        # 运行之后的初始化(ession.run(init))，损失变量将被初始化并准备计算
...
# # 并不是正确答案20，这一步只是把这两个变量放进了计算图中，没有立刻计算，需要通过会话进行
#
# sess = tf.Session()
# print(sess.run(c))
#
...
def initialize_parameters():
    ...
    # tf2里不再支持contrib，glorotuniform和Xavier初始化是一个东西
...
#     print("Z3 = " + str(Z3))
# 前向传播不会输出任何cache???是不是因为没有拿数据feed
